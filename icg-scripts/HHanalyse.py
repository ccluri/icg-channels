...
def main():
    ...
    for temperature in temperatures:
        ...
        volts = [-100, -90, -80, -78, -76, -74, -72, -70, -68, -66, -64, -62, -60, -58, -56,
                 -54, -52, -50, -48, -46, -44, -42, -40, -38, -36, -34, -32, -30,
                 -28, -26, -24, -22, -20, -18, -16, -14, -12, -10,  -8,  -6,  -4,
                 -2,   0,   2,   4,   6,   8, 10, 20, 30, 40, 50, 60, 70, 80, 100]
        ...
        if ca_present:
            pylab.get_current_fig_manager().set_window_title(
                "Steady state(s) of activation variables in %s at %s degC, %s mM caConc"
                % (chanToTest, h.celsius, '{:.0e}'.format(args.caConc)))
        else:
            pylab.get_current_fig_manager().set_window_title(
        ...
        if ca_present:
            pylab.get_current_fig_manager().set_window_title(
                "Time course(s) of activation variables in %s at %s degC, %s mM caConc"
                % (chanToTest, h.celsius, '{:.0e}'.format(args.caConc)))
        else:
            pylab.get_current_fig_manager().set_window_title(
        ...
        for s in states:
            if ca_present:
                file_name = "%s.%s.%s.inf%s.dat" % (chanToTest, s,
                                                    '{:.0e}'.format(args.caConc), temp_info)
            else:
            ...
            if ca_present:
                file_name = "%s.%s.%s.tau%s.dat" % (chanToTest, s,
                                                    '{:.0e}'.format(args.caConc), temp_info)
            else:
