...
def HHAnalyse_files(sub_chan_folder, path_to_hhanalyse,
                    ca_depd=False):
    with open(sub_chan_folder+'.pkl', 'rb') as fx:
        data_dict = pickle.load(fx)
    ...
    for ii, folder in enumerate(all_folders):
        if folder in ['.git', 'LICENSE', 'Readme.md', '.gitignore']:
            ...
            try:
                ...
                if suffix:
                    exist_hhdata = len(glob('*.dat'))
                    expect_hhdata = len(data_dict[folder]['STATES'])
                    if (exist_hhdata == 6*expect_hhdata) or (exist_hhdata == 18*expect_hhdata):
                        good_files += 1
                        print(folder, 'already obtained')
                        os.chdir('../..')
                        continue
                    a = path_to_hhanalyse
                    b = suffix
                    c = '-modFile='+fname
                    d = '-temperature=[37,23,6.3]'  # purposefully no spaces in list here!
                    e = '-nogui'
                    f = '-duration=100000'
                    if ca_depd:
                        for caconc in [5e-5, 5e-4, 5e-3]:
                            g = '-caConc='+str(caconc)
                            result = call_subprocess([a, b, c, d, e, f, g])
                    else:
