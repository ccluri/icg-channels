...
def update_functions(SM_NUM):
    ...
    return ss_fit_fcn_list, n_ss_params, ss_identity, ss_transitions, tau_fit_fcn_list, n_tau_params, tau_transitions, tau_identity
...
def fit_curves(data_dict, n_mod_files, SM_NUM, temp, ca_string=None):
    maxfev = 30000  # maximum number of iterations for scipy nonlinear fit fcn
    sm_str = "SM"+str(SM_NUM)
    ss_fit_fcn_list, n_ss_params, ss_identity, ss_transitions, tau_fit_fcn_list, n_tau_params, tau_transitions, tau_identity = update_functions(SM_NUM)
        ...
        try:
            ...
            if len(err_flags) == 0:
                ...
                for kk in valid_states:
                    if ca_string is not None:
                        fs = glob(rate_path + '/*.' + kk + '.' + ca_string + '.' + '*' + '.'+temp+'.dat')
                    else:
                        fs = glob(rate_path + '/*.' + kk + '*.' + temp + '.dat')
                    for fff in fs:
                ...
                for j in range(len(rate_files)):
                    ...
                    d = np.loadtxt(rfile, dtype=float)
                    V = d[:, 0]
                    rate_data = d[:, 1]
...
if __name__ == '__main__':
    ...
    iontype = sub_chan_folder.split('-')[-1]
    print(iontype)
    if iontype in ['KCa']:
        ca_string = '5e-03'
    else:
    ...
    backup(iontype, data_dict)
    print("Running for",iontype,"models.")
    print("Number of mod files: ", n_mod_files)
    for ii in range(1, 6):
        print('£'*100, ii)
        fit_curves(data_dict, n_mod_files, SM_NUM=ii, temp='6.3', ca_string=ca_string)
